...
const MedicalRecordPage = () => {
    // Lấy thêm hàm toggleVisibility ra để dùng
    const { records, loading, error, currentUser, toggleVisibility } = useMedicalRecords();
    const [selectedRecord, setSelectedRecord] = useState(null);

    if (loading) {
        return <div style={styles.loading}>Đang tải danh sách bệnh án...</div>;
    }
    ...
    if (!currentUser) {
        return <div style={styles.loading}>Vui lòng đăng nhập!</div>;
    }
        ...
        <div style={styles.container}>
            ...
            <table style={styles.table}>
                ...
                <tr style={styles.thRow}>
                    ...
                    {/* THÊM CỘT MỚI: Chỉ hiển thị cho bác sĩ */}
                    {currentUser.role === 'DOCTOR' && <th style={styles.th}>Quyền Bệnh Nhân</th>}
                ...
                {records.length > 0 ? (
                    records.map((item) => (
                        <tr key={item.id} style={styles.tr}>
                            <td style={styles.td} onClick={() => setSelectedRecord(item)}>BA-{item.id}</td>

                            {currentUser.role === 'DOCTOR' && <td style={styles.td} onClick={() => setSelectedRecord(item)}>{item.patientName || 'N/A'}</td>}

                            <td style={styles.td} onClick={() => setSelectedRecord(item)}>
                                <strong>{item.diagnosis}</strong>
                            </td>

                            <td style={styles.td} onClick={() => setSelectedRecord(item)}>{item.visitDate}</td>

                            {/* NÚT BẤM BẬT/TẮT: Chỉ hiển thị cho bác sĩ */}
                            {currentUser.role === 'DOCTOR' && (
                                <td style={styles.td}>
                                    <button
                                        style={item.isShared ? styles.sharedBtn : styles.hiddenBtn}
                                        onClick={(e) => {
                                            e.stopPropagation(); // Không cho mở popup chi tiết khi đang bấm nút này
                                            toggleVisibility(item.id, !item.isShared);
                                        }}
                                    >
                                        {item.isShared ? "Đã Mở " : "Đang Khóa "}
                                    </button>
                                </td>
                            )}

                            <td style={styles.td}>
                                <button style={styles.viewBtn} onClick={() => setSelectedRecord(item)}>Xem chi tiết</button>
                            </td>
                        </tr>
                    ))
                ) : (
                    <tr>
                        <td colSpan={currentUser.role === 'DOCTOR' ? 6 : 4} style={styles.noData}>
                ...
                <div style={styles.modalOverlay}>
                    <div style={styles.modalContent}>
                        ...
                        <div style={styles.modalBody}>
                            ...
                            <div style={styles.detailBox}>{selectedRecord.prescription || "Đang cập nhật..."}</div>

                            <p style={styles.detailItem}><strong> Lời dặn của Bác sĩ:</strong></p>
                            <div style={styles.detailBox}>{selectedRecord.doctorNotes || "Đang cập nhật..."}</div>
                        </div>
                    </div>
                </div>
            )}
...
// CSS (Đã thêm style cho nút bật/tắt)
const styles = {
    ...
    viewBtn: { backgroundColor: '#3498db', color: 'white', border: 'none', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer' },

    // Nút Bật/Tắt
    sharedBtn: { backgroundColor: '#2ecc71', color: 'white', border: 'none', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' },
    hiddenBtn: { backgroundColor: '#95a5a6', color: 'white', border: 'none', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' },
    ...
    error: { textAlign: 'center', marginTop: '50px', fontSize: '18px', fontWeight: 'bold', color: '#fff', padding: '20px', backgroundColor: '#e74c3c', borderRadius: '8px', maxWidth: '600px', margin: '50px auto' },
