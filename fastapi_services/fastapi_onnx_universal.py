"""
FastAPI Service - ONNX Universal (.onnx)
==========================================
Deploy YOLO26s-seg model bằng ONNX Runtime (CPU fallback).
Chạy được trên MỌI máy tính - Không yêu cầu GPU đặc biệt.

Chạy server:
    uvicorn fastapi_onnx_universal:app --host 0.0.0.0 --port 8004

Spring Boot gọi endpoint:
    POST http://localhost:8004/predict  (multipart/form-data, field "file")
    POST http://localhost:8004/predict/path (JSON body: {"image_path": "..."})
"""
...
# ========================== CẤU HÌNH ==========================
MODEL_PATH = r"D:\Hieu\Project_SWP391\YOLO26s-seg\best.onnx"  # TODO: Thay đổi đường dẫn thực tế
DEFAULT_CONF = 0.25
...
async def lifespan(app: FastAPI):
    """Load model ONNX Universal khi server khởi động."""
    global model
    from ultralytics import YOLO

    print(f"🔄 Đang nạp model ONNX (Universal) từ: {MODEL_PATH}")
    ...
    print(f"✅ Model ONNX (Universal Fallback) đã sẵn sàng!")
...
app = FastAPI(
    title="YOLO26s-seg | ONNX Universal API",
    description="API phát hiện u xơ tử cung (Myoma) từ ảnh siêu âm - Backend: ONNX Universal (CPU)",
    version="1.0.0",
    lifespan=lifespan,
)
...
@app.get("/health")
def health_check():
    return {"status": "healthy", "backend": "ONNX Universal (.onnx)", "model_loaded": model is not None}
...
async def predict_upload(
    ...
    """Nhận ảnh upload từ Spring Boot, chạy ONNX Universal inference."""
...
def predict_path(request: ImagePathRequest):
    """Nhận đường dẫn ảnh từ Spring Boot, chạy ONNX Universal inference."""
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("fastapi_onnx_universal:app", host="0.0.0.0", port=8004, reload=False)
