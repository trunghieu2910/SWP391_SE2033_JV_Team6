"""
FastAPI Service - ONNX DirectML (.onnx)
========================================
Deploy YOLO26s-seg model bằng ONNX Runtime + DirectML.
Dùng cho Windows với bất kỳ GPU nào (Intel, AMD, NVIDIA) qua DirectX 12.

Chạy server:
    uvicorn fastapi_onnx_directml:app --host 0.0.0.0 --port 8003

Spring Boot gọi endpoint:
    POST http://localhost:8003/predict  (multipart/form-data, field "file")
    POST http://localhost:8003/predict/path (JSON body: {"image_path": "..."})
"""
...
async def lifespan(app: FastAPI):
    """Load model ONNX DirectML khi server khởi động."""
    global model
    from ultralytics import YOLO

    print(f"🔄 Đang nạp model ONNX (DirectML) từ: {MODEL_PATH}")
    ...
    print(f"✅ Model ONNX (DirectML) đã sẵn sàng!")
...
app = FastAPI(
    title="YOLO26s-seg | ONNX DirectML API",
    description="API phát hiện u xơ tử cung (Myoma) từ ảnh siêu âm - Backend: ONNX + DirectML",
    version="1.0.0",
    lifespan=lifespan,
)
...
@app.get("/health")
def health_check():
    return {"status": "healthy", "backend": "ONNX DirectML (.onnx)", "model_loaded": model is not None}
...
async def predict_upload(
    ...
    """Nhận ảnh upload từ Spring Boot, chạy ONNX DirectML inference."""
...
def predict_path(request: ImagePathRequest):
    """Nhận đường dẫn ảnh từ Spring Boot, chạy ONNX DirectML inference."""
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("fastapi_onnx_directml:app", host="0.0.0.0", port=8003, reload=False)
