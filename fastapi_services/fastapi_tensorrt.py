"""
FastAPI Service - TensorRT (.engine)
=====================================
Deploy YOLO26s-seg model bằng TensorRT.
Dùng cho máy có NVIDIA GPU + CUDA + TensorRT đã cài đặt.

Chạy server:
    uvicorn fastapi_tensorrt:app --host 0.0.0.0 --port 8001

Spring Boot gọi endpoint:
    POST http://localhost:8001/predict  (multipart/form-data, field "file")
    POST http://localhost:8001/predict/path (JSON body: {"image_path": "..."})
"""
...
# ========================== CẤU HÌNH ==========================
MODEL_PATH = r"D:\Hieu\Project_SWP391\YOLO26s-seg\best.engine"  # TODO: Thay đổi đường dẫn thực tế
DEFAULT_CONF = 0.25
...
async def lifespan(app: FastAPI):
    """Load model TensorRT vào VRAM khi server khởi động."""
    global model
    from ultralytics import YOLO

    print(f"🔄 Đang nạp model TensorRT từ: {MODEL_PATH}")
    if not os.path.exists(MODEL_PATH) or not MODEL_PATH.endswith(".engine"):
        raise FileNotFoundError(f"Không tìm thấy file .engine: {MODEL_PATH}")

    model = YOLO(MODEL_PATH)

    # Warmup CUDA + TensorRT context
    dummy = np.zeros((640, 640, 3), dtype=np.uint8)
    _ = model(dummy, conf=0.99, verbose=False)
    print(f"✅ Model TensorRT đã sẵn sàng!")
...
app = FastAPI(
    title="YOLO26s-seg | TensorRT API",
    description="API phát hiện u xơ tử cung (Myoma) từ ảnh siêu âm - Backend: TensorRT",
    version="1.0.0",
    lifespan=lifespan,
)
...
@app.get("/health")
def health_check():
    return {"status": "healthy", "backend": "TensorRT (.engine)", "model_loaded": model is not None}
...
async def predict_upload(
    ...
    """Nhận ảnh upload từ Spring Boot, chạy TensorRT inference."""
...
def predict_path(request: ImagePathRequest):
    """Nhận đường dẫn ảnh từ Spring Boot, chạy TensorRT inference."""
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("fastapi_tensorrt:app", host="0.0.0.0", port=8001, reload=False)
