"""
FastAPI Service - OpenVINO (folder model)
==========================================
Deploy YOLO26s-seg model bằng Intel OpenVINO.
Dùng cho máy có Intel CPU/GPU (iGPU) - Tối ưu cho Intel hardware.

Chạy server:
    uvicorn fastapi_openvino:app --host 0.0.0.0 --port 8002

Spring Boot gọi endpoint:
    POST http://localhost:8002/predict  (multipart/form-data, field "file")
    POST http://localhost:8002/predict/path (JSON body: {"image_path": "..."})
"""
...
# ========================== CẤU HÌNH ==========================
MODEL_PATH = r"D:\Hieu\Project_SWP391\YOLO26s-seg\best_openvino_model"  # TODO: Thay đổi đường dẫn thực tế (folder)
DEFAULT_CONF = 0.25
...
async def lifespan(app: FastAPI):
    """Load model OpenVINO khi server khởi động."""
    global model
    from ultralytics import YOLO

    print(f"🔄 Đang nạp model OpenVINO từ: {MODEL_PATH}")
    if not os.path.exists(MODEL_PATH) or not os.path.isdir(MODEL_PATH):
        raise FileNotFoundError(
            f"Không tìm thấy thư mục model OpenVINO: {MODEL_PATH}\n"
            f"Hãy chạy: python export_openvino.py --weights best.pt"
        )
    ...
    print(f"✅ Model OpenVINO đã sẵn sàng!")
...
app = FastAPI(
    title="YOLO26s-seg | OpenVINO API",
    description="API phát hiện u xơ tử cung (Myoma) từ ảnh siêu âm - Backend: OpenVINO",
    version="1.0.0",
    lifespan=lifespan,
)
...
@app.get("/health")
def health_check():
    return {"status": "healthy", "backend": "OpenVINO (folder)", "model_loaded": model is not None}
...
async def predict_upload(
    ...
    """Nhận ảnh upload từ Spring Boot, chạy OpenVINO inference."""
...
def predict_path(request: ImagePathRequest):
    """Nhận đường dẫn ảnh từ Spring Boot, chạy OpenVINO inference."""
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("fastapi_openvino:app", host="0.0.0.0", port=8002, reload=False)
