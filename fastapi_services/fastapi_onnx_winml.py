"""
FastAPI Service - ONNX WinML (.onnx)
======================================
Deploy YOLO26s-seg model bằng ONNX Runtime + Windows ML.
Dùng cho Windows 10/11 với GPU tích hợp hoặc rời qua Windows ML API.

Chạy server:
    uvicorn fastapi_onnx_winml:app --host 0.0.0.0 --port 8005

Spring Boot gọi endpoint:
    POST http://localhost:8005/predict  (multipart/form-data, field "file")
    POST http://localhost:8005/predict/path (JSON body: {"image_path": "..."})
"""
...
async def lifespan(app: FastAPI):
    """Load model ONNX WinML khi server khởi động."""
    global model
    from ultralytics import YOLO

    print(f"🔄 Đang nạp model ONNX (WinML) từ: {MODEL_PATH}")
    ...
    print(f"✅ Model ONNX (WinML) đã sẵn sàng!")
...
app = FastAPI(
    title="YOLO26s-seg | ONNX WinML API",
    description="API phát hiện u xơ tử cung (Myoma) từ ảnh siêu âm - Backend: ONNX + WinML",
    version="1.0.0",
    lifespan=lifespan,
)
...
@app.get("/health")
def health_check():
    return {"status": "healthy", "backend": "ONNX WinML (.onnx)", "model_loaded": model is not None}
...
async def predict_upload(
    ...
    """Nhận ảnh upload từ Spring Boot, chạy ONNX WinML inference."""
...
def predict_path(request: ImagePathRequest):
    """Nhận đường dẫn ảnh từ Spring Boot, chạy ONNX WinML inference."""
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("fastapi_onnx_winml:app", host="0.0.0.0", port=8005, reload=False)
